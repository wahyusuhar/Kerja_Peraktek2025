...
<div class="modal fade" id="modal-supplier" tabindex="-1" role="dialog" aria-labelledby="modal-supplier">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-header bg-white position-relative" style="padding: 0;">
                ...
                <div class="w-100 d-flex justify-content-center align-items-center py-3">
                    <h5 class="modal-title text-white text-center px-4 py-2 mb-0"
                        ...
                        ">
                        <i class="fa fa-truck me-2"></i> Pilih Supplier
                    </h5>
                </div>
            </div>
            ...
            style="box-shadow: rgba(50, 50, 93, 0.25) 0px 13px 27px -5px,
                   rgba(0, 0, 0, 0.3) 0px 8px 16px -8px; border-radius: 8px;">
            <table class="table table-striped table-bordered table-supplier">
                <thead>
                    <tr>
                        <th width="10%" style="background-color: #322fe1; color: white;">No</th>
                        <th style="background-color: #322fe1; color: white;">Nama Supplier</th>
                ...
                <tbody>
                    @foreach ($supplier as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->nama }}</td>
                            <td>{{ $item->telepon }}</td>
                            <td style="white-space: normal; word-wrap: break-word; max-width: 230px;">
                                {{ $item->alamat }}
                            </td>
                            <td>
                                {{-- <a href="#" class="btn btn-primary btn-sm btn-flat"
                                    onclick="pilihSupplier('{{ $item->id_supplier }}', '{{ $item->nama }}')">
                                    <i class="fa fa-check-circle"></i>
                                    Pilih
                                </a> --}}
                                <a href="{{ route('pembelian.create', $item->id_supplier) }}" class="btn btn-primary btn-sm btn-flat"> <i class="fa fa-check-circle"></i> Pilih </a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
